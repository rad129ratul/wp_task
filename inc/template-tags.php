...
if ( ! function_exists( 'ct_custom_posted_on' ) ) :
	...
	function ct_custom_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'ct-custom' ),
...
if ( ! function_exists( 'ct_custom_posted_by' ) ) :
	...
	function ct_custom_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'ct-custom' ),
	...
	function ct_custom_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'ct-custom' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'ct-custom' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'ct-custom' ),
						array(
							'span' => array(
								'class' => array(),
							),
